...
packages <- c("tidyverse", "flextable", "psych", "irr")
...
day <- data %>%
  ...
  dplyr::summarise(sum_sm = sum(phone_applications_foreground_rapids_sumdurationsm, na.rm = TRUE), NAf_am = mean(Mood_1_am, na.rm=TRUE), NAf_pm = mean(Mood_1_pm, na.rm=TRUE), count_sm = sum(phone_applications_foreground_rapids_countsm, na.rm = TRUE), SM_Pos = mean(SMPos_1_pm, na.rm=TRUE), SM_Neg = mean(SMNeg_1_pm, na.rm=TRUE)) %>%
...
SM_time <- icc("SM_time", data, "hour_in_study", "phone_applications_foreground_rapids_sumdurationsm")
SM_count <- icc("SM_time", data, "hour_in_study", "phone_applications_foreground_rapids_countsm")
...
demog = data.frame(
  ...
  Measure = c("Age, M (SD)", "Race", "White", "Black/African American", "Asian", "Ethnicity", "Hispanic/Latine", "Non-Hispanic/Latine", "Sex", "Male", "Female", "Gender", "Boys", "Girls", "Non-binary/third gender", "Transgender", "Sexual Orientation", "Heterosexual/Straight", "Bisexual", "Queer", "Bi-curious", "I don't know", "Socioeconomic Status - Society, M (SD; Range)", "Socioeconomic Status - School, M (SD; Range)"),
...
n = c("", paste(n_distinct(day)-n_distinct(which(is.na(day$NAf_am)))),  paste(n_distinct(day)-n_distinct(which(is.na(day$NAf_pm)))),  paste(n_distinct(day)-n_distinct(which(is.na(day$SM_Pos)))), paste(n_distinct(day)-n_distinct(which(is.na(day$SM_Neg)))), "", paste(n_distinct(data)-n_distinct(which(is.na(data$phone_applications_foreground_rapids_sumdurationsm)))), paste(n_distinct(data)-n_distinct(which(is.na(data$phone_applications_foreground_rapids_countsm))))),
...
ICC = c("", round(morning$results$ICC[3], 2), round(evening$results$ICC[3], 2), round(pos$results$ICC[3], 2), round(neg$results$ICC[3], 2), "", round(time$results$ICC[3], 2), round(count$results$ICC[3], 2))
