...
day <- day %>% group_by(pid) %>%
  dplyr::mutate(sum_sm_p_lag = lag(sum_sm_p, n = 1, default = NA), count_sm_p_lag = lag(count_sm_p, n = 1, default = NA), NAf_am_p_lag = lag(NAf_am_p, n = 1, default = NA), NAf_pm_p_lag = lag(NAf_pm_p, n = 1, default = NA))
...
NA_sm_sum_bayes  <- brm(NAf_am_p_lag ~ sum_sm_p + NAf_pm_p + sum_sm_p_c + day_in_study + (1 | pid), prior = prior1, data = day)
...
NA_sm_count_bayes  <- brm(NAf_am_p_lag ~ count_sm_p + NAf_pm_p + count_sm_p_c + day_in_study + (1 | pid), prior = prior1, data = day)
...
PA_on_SM_day_bayes  <- brm(sum_sm_p_lag ~ SM_Pos_p + SM_Pos_p_c + day_in_study + (1 | pid), prior = prior1, data = day)
...
PA_on_SM_count_day_bayes  <- brm(count_sm_p_lag ~ SM_Pos_p + SM_Pos_p_c + day_in_study + (1 | pid), prior = prior1, data = day)
...
NA_on_SM_day_bayes  <- brm(sum_sm_p_lag ~ SM_Neg_p + SM_Neg_p_c + day_in_study + (1 | pid), prior = prior1, data = day)
...
NA_on_SM_count_day_bayes  <- brm(count_sm_p_lag ~ SM_Neg_p + SM_Neg_p_c + day_in_study + (1 | pid), prior = prior1, data = day)
